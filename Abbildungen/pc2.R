# Installiere ggplot2, wenn es noch nicht installiert ist
# install.packages("ggplot2")

# Lade das ggplot2-Paket
library(ggplot2)

# Erstelle Daten mit mindestens 10 Punkten
set.seed(123)  # Für die Reproduzierbarkeit der Zufallsdaten
x <- 1:10
y <- 2*x + rnorm(10)

# Erstelle ein Datenrahmen
data <- data.frame(x, y)

# Führe eine lineare Regression durch
lm_model <- lm(y ~ x, data=data)

# Projiziere die Punkte auf die Gerade
data$y_proj <- predict(lm_model, data)

# Berechne die Varianz der auf die Gerade projizierten Punkte
var_proj <- var(data$y_proj)

# Mittelpunkt der ursprünglichen Geraden
midpoint <- c(mean(data$x), mean(data$y))

# Steigung der orthogonalen Geraden
slope_ortho <- -1 / coef(lm_model)[2]

# Y-Achsenabschnitt der orthogonalen Geraden
intercept_ortho <- midpoint[2] - slope_ortho * midpoint[1]

# Projiziere die Datenpunkte auf die zweite Gerade
data$y_proj_second <- slope_ortho * data$x + intercept_ortho

# Berechne die Varianz der auf die zweite Gerade projizierten Punkte
var_proj_second <- var(data$y_proj_second)

# Erstelle die Plot-Grafik
ggplot(data, aes(x, y)) +
  geom_point(color = "#115E67", size = 3, shape = 16) +  # Füge Punkte hinzu und passe Farbe, Größe und Form an
  geom_abline(intercept = coef(lm_model)[1], slope = coef(lm_model)[2], linetype="solid", color="#D9C756") +  # Füge die ursprüngliche Gerade hinzu und passe Farbe und Linientyp an
  geom_abline(intercept = intercept_ortho, slope = slope_ortho, color="#BE531C") +  # Füge die orthogonale Gerade hinzu
  geom_segment(aes(x = x, y = y, xend = x, yend = y_proj_second), color = "#8FD6BD") +  # Füge die Projektionslinien hinzu und passe Farbe an
  labs(x="X-Achse",
       y="Y-Achse") +
  theme_minimal() +  # Wähle ein minimalistisches Design
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),  # Zentriere und passe die Größe des Titels an
        axis.title = element_text(size = 12, face = "bold"),  # Passe die Größe der Achsentitel an
        axis.text = element_text(size = 10),  # Passe die Größe der Achsentexte an
        legend.position="none")  # Entferne die Legende (da wir keine spezifischen Legendeninformationen haben)
