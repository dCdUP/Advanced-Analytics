...
ggplot(data, aes(x, y)) +
  ...
  geom_segment(aes(x = x, y = y, xend = x, yend = y_proj), color = "#8FD6BD") +  # Füge die Projektionslinien hinzu und passe Farbe an
  labs(title = paste("Varianz",round(var_proj,digits = 2),sep = " "),
       x="X-Achse",
       y="Y-Achse") +
